...
const schema = z.object({
  ...
  operator:            z.string().min(2, 'Required'),
  train_number:        z.string().min(1, 'Required'),
  origin_city:         z.string().min(2, 'Required'),
  origin_station:      z.string().min(2, 'Required'),
  destination_city:    z.string().min(2, 'Required'),
  destination_station: z.string().min(2, 'Required'),
...
export function TrainForm({ countries, defaultValues, listingId }: Props) {
  ...
  const onSubmit = async (data: FormData) => {
    const { operator, train_number, origin_city, origin_station, destination_city, destination_station, duration_minutes, ...rest } = data
    const payload = {
      ...rest,
      type: 'TRAIN',
      train: { operator, train_number, origin_city, origin_station, destination_city, destination_station, duration_minutes },
    }
    const res = listingId
    ...
    if (res.ok) router.push('/trains')
  }

  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <Link href="/trains" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700">
      ...
      <div className="space-y-4">
        <div>
          ...
          <input {...register('title')} placeholder="Rajdhani Express — Mumbai to Delhi"
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div className="flex">
              ...
              <input {...register('base_price')} type="number" step="0.01" placeholder="45"
      ...
      <div className="space-y-4">
        <h3 className="text-sm font-semibold text-gray-700">Train details</h3>
        <div className="grid grid-cols-3 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Operator <span className="text-red-500">*</span></label>
            <input {...register('operator')} placeholder="Indian Railways" className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-brand-500 bg-white" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Train number <span className="text-red-500">*</span></label>
            <input {...register('train_number')} placeholder="12951" className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-brand-500 bg-white" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Duration (mins) <span className="text-red-500">*</span></label>
            <input {...register('duration_minutes')} type="number" placeholder="960" className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-brand-500 bg-white" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">From station <span className="text-red-500">*</span></label>
            <input {...register('origin_station')} placeholder="Chhatrapati Shivaji Maharaj Terminus" className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-brand-500 bg-white" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">To city <span className="text-red-500">*</span></label>
            <input {...register('destination_city')} placeholder="New Delhi" className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-brand-500 bg-white" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">To station <span className="text-red-500">*</span></label>
            <input {...register('destination_station')} placeholder="New Delhi Railway Station" className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-brand-500 bg-white" />
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-3 pt-2">
        ...
        <Link href="/trains" className="px-6 py-2.5 rounded-xl border border-gray-200 text-sm font-medium text-gray-600 hover:bg-gray-50">Cancel</Link>
      </div>
    </form>
  )
}
